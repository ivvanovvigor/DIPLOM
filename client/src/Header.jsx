...
const Header = ({ cartItems = [], favoriteItems = [], removeFromCart, updateQuantity, onLogout }) => {
  ...
  // === Оновлення лічильників при зміні props ===
      ...
      <div className="w-full flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4 sm:space-x-6 flex-shrink-0">
          ...
          <Link to="/favorites" className="text-xs font-black uppercase tracking-widest text-black hover:opacity-70 transition flex items-center">
            <svg className="h-6 w-6 fill-black mr-1" viewBox="0 0 24 24">
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
            </svg>
            <span className="hidden md:inline">Обране</span>
            {favCount > 0 && <span className="bg-gray-100 border border-black px-1.5 ml-1">{favCount}</span>}
          </Link>
          ...
          <div className="relative">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1.5 bg-black text-white text-[9px] font-bold px-1.5 rounded-full">
                  {cartCount}
                </span>
              )}
            </button>
          ...
          {user || token ? (
            <div className="flex items-center space-x-4 border-l pl-4">
              <Link to="/profile" className="text-xs font-bold uppercase hover:underline">Кабінет</Link>
              <button
            ...
            <Link to="/login" className="text-xs font-bold uppercase border-l pl-4 hover:underline">Увійти</Link>
          )}
        </div>
      </div>
    </header>
